	...
	showOnClick, sepItems, isMainMenu, hasAnimations, animationType, hasShadow, sOffX, sOffY, shadowColor)
{
	...
	{
		this.main.style.height = (size + margin) + "px";
		if (sepItems)
	...
	else
	{
		...
		this.main.style.width = (size + margin) + "px";
	}
	if (isMainMenu)
...
function item(link, target, iSize, alignment, content, menuToShow)
{
	...
	temp.style.paddingLeft = this.margin + "px";
	temp.style.paddingTop = this.margin + "px";
	temp.style.position = "absolute";
	if (this.orientation == "horizontal")
	{
		temp.style.left = this.position + "px";
	...
	{
		if (this.orientation == "horizontal")
			this.main.style.width = (this.main.offsetWidth + iSize + this.borderSize + this.margin) + "px";
		else
			this.main.style.height = (this.main.offsetHeight + iSize + this.borderSize + this.margin) + "px";
		this.position += iSize + this.borderSize + this.margin;
	}
	else
	{
		if (this.orientation == "horizontal")
			this.main.style.width = (this.main.offsetWidth + iSize + this.margin) + "px";
		else
			this.main.style.height = (this.main.offsetHeight + iSize + this.margin) + "px";
...
function showMenu(num, pNum, iNum, offsetX, offsetY)
{
	if (menus[num].main.style.visibility != "visible")
	{
		...
		if (!docType && showX + menus[num].main.offsetWidth > bodyRef.clientWidth + bodyRef.scrollLeft)
			showX -= (showX + menus[num].main.offsetWidth) - (bodyRef.clientWidth + 
				bodyRef.scrollLeft);
		if (!docType && showY + menus[num].main.offsetHeight > bodyRef.clientHeight + bodyRef.scrollTop)
			showY -= (showY + menus[num].main.offsetHeight) - (bodyRef.clientHeight + 
				bodyRef.scrollTop);
